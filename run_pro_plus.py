...
def detect_rsi_patterns(csv_file_path):
    ...
    # Lấy 100 nến gần nhất (khoảng 1 tuần giao dịch thực tế, bỏ qua thời gian đóng cửa)
    df_recent = df.tail(100).reset_index(drop=True)
    df_recent["_dt"] = pd.to_datetime(df_recent["Time"], errors="coerce")
    
    short_labels = []
    prev_day = None
    for dt in df_recent["_dt"]:
        if pd.isna(dt):
            short_labels.append("")
            continue
            
        hm = f"{dt.hour}:{dt.minute:02d}"
        # Đổi ngày -> thêm ngày vào label (VD: 16-7:00)
        if prev_day is None or dt.day != prev_day:
            short_labels.append(f"{dt.day}-{hm}")
        else:
            short_labels.append(hm)
        prev_day = dt.day

    labels = short_labels
    
    clean_rsi = []
    for x in df_recent["RSI"]:
        try:
            clean_rsi.append(round(float(x), 2))
        except:
    ...
    local_min = []
    win = 2
    for i in range(win, len(clean_rsi) - win):
        if all(clean_rsi[i] > clean_rsi[i-k] for k in range(1, win+1)) and \
           all(clean_rsi[i] > clean_rsi[i+k] for k in range(1, win+1)):
            local_max.append(i)
        if all(clean_rsi[i] < clean_rsi[i-k] for k in range(1, win+1)) and \
           all(clean_rsi[i] < clean_rsi[i+k] for k in range(1, win+1)):
            local_min.append(i)

    # W patterns (double bottom)
    if len(local_min) >= 2:
        for i in range(len(local_min) - 1):
            idx1, idx2 = local_min[i], local_min[i+1]
            val1, val2 = clean_rsi[idx1], clean_rsi[idx2]
            if 3 <= (idx2 - idx1) <= 30:
                peaks_between = [p for p in local_max if idx1 < p < idx2]
                if peaks_between:
                    peak_val = max(clean_rsi[p] for p in peaks_between)
                    if peak_val - min(val1, val2) > 5 and abs(val1 - val2) < 12:
                        mid = max(peaks_between, key=lambda p: clean_rsi[p])
                        patterns.append({
                            "type": "W",
                            "start_idx": int(idx1), "mid_idx": int(mid), "end_idx": int(idx2),
                            "start_time": labels[idx1], "end_time": labels[idx2],
                            "start_val": float(val1), "mid_val": float(clean_rsi[mid]), "end_val": float(val2)
                        })
                        if idx2 >= len(clean_rsi) - 8 and clean_rsi[-1] > clean_rsi[idx2]:
                            is_forming = 'W'

    # M patterns (double top)
    if len(local_max) >= 2:
        for i in range(len(local_max) - 1):
            ...
            if 3 <= (idx2 - idx1) <= 30:
                ...
                if troughs_between:
                    ...
                    if max(val1, val2) - trough_val > 5 and abs(val1 - val2) < 12:
...
def run_pro_plus():
    ...
    # 4.1. Nạp Weather Data (Mới từ hệ thống tự động Data)
    weather_data = {"regions": {}, "long_term": {}}
    try:
        with open(_data_path("weather_short.json"), "r", encoding="utf-8") as f:
            ws = json.load(f)
            weather_data["regions"] = ws.get("regions", {})
    except Exception: pass
    try:
        with open(_data_path("weather_long.json"), "r", encoding="utf-8") as f:
            wl = json.load(f)
            weather_data["long_term"] = wl
    except Exception: pass

    # 4.2. Nạp Export Sales (Mới từ hệ thống tự động Data)
    export_data = {}
    try:
        with open(_data_path("export_sales.json"), "r", encoding="utf-8") as f:
    ...
    report_date_str = "N/A"
    for cdata in cot_data_json.get("commodities", {}).values():
        if "report_date" in cdata:
            report_date_str = cdata["report_date"]
            break
...
*Cập nhật từ nguồn CFTC Public API (Ngày báo cáo chốt sổ: **{report_date_str}**)*
...
"""
    ...
    zc_swing_long_entry = f"{zc_sw['s1']-1.5:.2f} - {zc_sw['s1']:.2f} cents"
    zc_swing_long_sl = f"{zc_sw['s2'] - 1.0 * zc_sw['atr']:.2f} cents (Chống quét SL tuyệt đối)"
    zc_swing_long_tp = f"{zc_sw['r2'] - 1.0:.2f} cents (Ăn trọn biên độ ngô ~10.5 giá)"
    zc_swing_short_entry = f"{zc_sw['r1']:.2f} - {zc_sw['r1']+1.5:.2f} cents"
    zc_swing_short_sl = f"{zc_sw['r1'] + 1.0 * zc_sw['atr']:.2f} cents"
    ...
    zw_swing_long_entry = f"{zw_sw['s1']-2.0:.2f} - {zw_sw['s1']:.2f} cents"
    zw_swing_long_sl = f"{zw_sw['s2'] - 1.0 * zw_sw['atr']:.2f} cents"
    zw_swing_long_tp = f"{zw_sw['r1']:.2f} cents (Đón sóng hồi trung hạn ~14.5 giá)"
    zw_swing_short_entry = f"{zw_sw['r1']:.2f} - {zw_sw['r1']+2.0:.2f} cents"
    zw_swing_short_sl = f"{zw_sw['r1'] + 1.0 * zw_sw['atr']:.2f} cents"
...
| :---: | :--- | :---: | :---: | :---: | :---: | :--- |
| **ZC** | Bán hàng & Giao hàng | {fund['ZC']['exports'].get('previous', 'N/A').replace('|', '\\|')} | **{fund['ZC']['exports'].get('latest', 'N/A').replace('|', '\\|')}** | {fund['ZC']['exports'].get('forecast', 'N/A')} | {fund['ZC']['exports'].get('next_date', 'N/A')} | {fund['ZC']['exports'].get('action', 'N/A')} - {fund['ZC']['exports'].get('logic', 'N/A')} |
| **ZW** | Bán hàng & Giao hàng | {fund['ZW']['exports'].get('previous', 'N/A').replace('|', '\\|')} | **{fund['ZW']['exports'].get('latest', 'N/A').replace('|', '\\|')}** | {fund['ZW']['exports'].get('forecast', 'N/A')} | {fund['ZW']['exports'].get('next_date', 'N/A')} | {fund['ZW']['exports'].get('action', 'N/A')} - {fund['ZW']['exports'].get('logic', 'N/A')} |
...
"""
    ...
    section_zc = build_commodity_section(
        ...
        weather_data=weather_data, export_data=export_data
    )
    ...
    report_content += phan3_header + section_zc + section_zw
